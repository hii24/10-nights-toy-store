...
## [P0-02 — ОТЛОЖЕНО / ВНЕШНИЙ БЛОК] CI на GitHub — 2026-05-31
...
- **Статус:** `P0-02` остаётся `passes:false`. Когда lock снимут — один push даст зелёный CI, фича закроется за минуту. Локальный `dev` почищен (CI-фикс одним коммитом `399bc77`); `origin/dev` пока с историей попыток — пере-синкнем при финализации.
