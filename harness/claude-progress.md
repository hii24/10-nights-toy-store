...
- **Коммит:** всё в working tree (дизайн-уровень + acc-свет + меш-медведь). РЕШАЕМ С ЮЗЕРОМ.
  - **[ОБНОВЛЕНО]** Зашиплено: коммит `d69c873` → merge `--no-ff` в `dev` (`79825bb`) → push. CI зелёный: Selene/StyLua/build + **TestEZ 69/0/0** (реальный Open Cloud-движок, v7). Локальный «67» был require-кэшем долгой edit-сессии — подтверждено.

---

## [P2-08 — passes:true] Свет день/ночь + туман + конусы фонариков — 2026-06-01
Автономно (юзер: «давай»). Полноценная атмосфера хоррора вместо временного мягкого фликера. Всё КЛИЕНТСКОЕ (пер-клиент визуал, не реплицируется — docs/01), драйв `Workspace.RoundState` + `GeneratorPowered` (decoupled, как Door/Upgrade-сервисы).
- **Сделано:** `src/shared/Config/LightingConfig.luau` (НОВЫЙ, data-driven: 3 пресета day/nightPowered/nightDark + alarm + flashlight + `readabilityFloor`; цвета строго из библии §3) · `LightingController.client.luau` (переписан: Atmosphere+ColorCorrection, твины Lighting по режиму, мягкий **Alarm-синус-пульс** только в ночь-тьму — НЕ строб) · `FlashlightController.client.luau` (НОВЫЙ: 1 SpotLight Warm White без теней, целится по камере в RenderStepped, вкл. ночью, идемпотентный эмиттер) · `tests/LightingConfig.spec.luau` (НОВЫЙ, 8 тестов).
- **Маппинг:** День (Prep/Morning/Upgrade/Lost…) = тёплый ClockTime 14.5, Brightness 2.2, warm ambient, фонарик OFF. Ночь+питание = синева, лампы горят (1.4, Power-Cyan-tilt). Ночь-тьма = deep-blue (ambient 30,36,60), плотный туман (fogEnd 80), Alarm-пульс, фонарик ON. Гарантия §3.1 «темно но НЕ слепо».
- **End-to-end в Studio (Play):** все 3 режима флипают корректно (LightMode-атрибут: NightPowered/NightDark/Day); день тёплый/яркий (скрин), ночь сине-тёмная + конус фонарика освещает пол, читаемо (скрин); фонарик OFF днём, ON ночью; переход nightPowered→nightDark **плавно стекает** (1.40→1.29→…→0.74, без снапа). TestEZ **75/0/0** (edit; +8 lighting, BearConfig в кэше=4; CI свежий = 77).
- **Review (performance-auditor): CONDITIONAL PASS → все находки закрыты.** Перф PASS (1 SpotLight без теней, нет per-lamp динам-света, alarm 0.33 Гц синус = в 9× ниже фото-порога, НЕ строб). Фиксы: **MUST-1** snap яркости при nightPowered→nightDark → alarm-петля теперь ЕДИНСТВЕННЫЙ владелец Brightness/Tint в ночь-тьму, плавный lerp-заход (подтверждено сэмплом); **MUST-2** readabilityFloor не enforced → рантайм-проверка на загрузке + CI-spec гейтит; **SHOULD-1** ambient (24,28,50)→(30,36,60) ближе к библии, контраст с Night Neutral полом; **SHOULD-2** идемпотентный эмиттер (ensure-паттерн); **NICE** кэш nightActive вне RenderStepped + `t += task.wait()` (FPS-независимо).
- **Локально:** Selene 0/0/0, StyLua ✓, `rojo build` ✓.
- **🔸 Хвосты:** реал-девайс тест (телефон mid/low-end, яркость 30-40%) — рекоменд. аудитора, формально P2-13; per-lamp аварийные PointLight'и (намеренно НЕ делал — перф) — опционально; GlobalShadows OFF (мобиль) — конус виден как засветка, тени = тюнинг-хвост; flashlight Face при инверсии → Back.
- **Фаза 2: 4/13** (P2-01/02/08 + меш-половина P2-03). **Следующее по приоритету — P2-04** (кастомные Doll+Soldier, ждёт модели юзера) либо контент/ассеты.
